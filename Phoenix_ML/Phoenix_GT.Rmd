...
mtry <- sqrt(ncol(training_p))
tunegrid <- expand.grid(.mtry=mtry)
model2 <- train(as.factor(samesource) ~ ccf
                 +rough_cor
                 +D
                 +sd_D
                 +matches
...
rf_pred2 <- predict(model2, testing_p)
confusionMatrix(rf_pred2, as.factor(testing_p$samesource))

```



Accuracy for both models is around the same.  Now we will try and find miss classified ground truth results by examining signatures between Barrel_Bullet_Land comparisons.
```{r}
...
mtry <- sqrt(ncol(training_p))
tunegrid <- expand.grid(.mtry=mtry)
mode1_3 <- train(as.factor(samesource) ~ ccf
                 +rough_cor
                 +D
                 +sd_D
                 +matches
...
rf_pred_3 <- predict(mode1_3, testing_p)
